...
with tab3:
    ...
    if existing:
        ...
        try:
            ...
            webrtc_streamer(key="face_registration", video_transformer_factory=FaceRegister)
        except ImportError:
...
with tab5:
    ...
    st.markdown("Mark yourself present using AI. The system physically cross-references the **1 MSc AIML** Class Timetable to unlock the camera.")
    ...
    # Overriding sunday for your local testing purposes
    if curr_day == "Sunday" or True: # Remove 'or True' in production
        active_subject = "Demo Session (For Evaluation)"
        st.success(f"🔓 Timetable Unlocked: {active_subject}")
    else:
        for block in MSC_TIMETABLE.get(curr_day, []):
            if block["start"] <= curr_time <= block["end"]:
                active_subject = block["subject"]
                break
                
        if not active_subject:
            st.error(f"🔒 Timetable Locked: There is currently no class scheduled right now ({curr_day} {curr_time}). Come back later!")
            
    if active_subject:
        # Load user data natively
        me = db.users.find_one({"email": email})
        ...
        if not my_embedding:
            ...
            try:
                ...
                class SelfAttendance(VideoTransformerBase):
                    def __init__(self):
                        self.detector = MTCNN()
                            ...
                            if res:
                                ...
                                if dist < 0.7:
                                    ...
                                    db.attendance_records.update_one(
                                        {"session_id": f"Self_{date_key}_{active_subject}"},
                                        {"$set": {"department": "MSc AIML", "subject": active_subject},
                                         "$addToSet": {"students": {"student_id": email, "name": name, "present": True, "timestamp": time.time()}}},
                                        upsert=True
                                    )
                                else:
                ...
                webrtc_streamer(key="self_attendance", video_transformer_factory=SelfAttendance)
            except ImportError:
